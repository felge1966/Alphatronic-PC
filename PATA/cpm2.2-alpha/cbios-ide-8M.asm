...
dpblk:	;disk parameter block for all disks.
	...
	defm	5		;BSF block shift factor -- 2^7=128 sec/block, 128*128=16K block size
	defm	31		;BLM block mask -- with block shift factor of 7, need 7 bits to count 128 sectors/block, so mask is 127
	defm	1		;EXM extend mask -- required for  DSM>256
	defw	2043	;DSM disk size-1-OFF = number of blocks in a disk - 1 - OFF
	defw	511		;DRM directory max = no. directory entries/disk, arbitrary
	defm	240		;AL0 alloc 0 -- need 4 bits (blocks) for 256 directory entries -- 
	defm	0		;AL1 alloc 1 -- no. bits = (directory max x 32)/block size	
...
greet:
	...
	defm	"CP/M 2.2 for TA Alphatronic PC 50K",0dh,0ah
	defm	"CBIOS v.1.1 with PATA/IDE 8M disks and IOBYTE support by JS",0dh,0ah
	defm	0dh, 0ah, 0	

;	individual subroutines to perform each function
boot:
	;entry point from CP/M loader
	...
	; initialize IOBYTE to default values for TA Alphatronic PC
...
all00:	defs	257	 	;allocation vector 0
all01:	defs	257	 	;allocation vector 1
all02:	defs	257	 	;allocation vector 2
all03:	defs	257	 	;allocation vector 3
...
hstbuf: defs	128		;buffer for host disk sector
