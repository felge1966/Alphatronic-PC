...
; - Disks have 128 sectors per track, 512 tracks, for a total of 65536 sectors per disk
...
cpmformat:
		ld	bc,directory_sector	;address of data to write
		ld	(address),bc
		call	setdma
...
disk_loop:
		ld	c,a				;CP/M disk a
		call	seldsk
		ld	bc,1			;starting track (offset = 1)
		ld	(track),bc
track_loop:
		ld	bc,0		;starting sector
		ld	(sector),bc
		ld	bc,(track)
		call	settrk
sector_loop:
...
putboot:
		...
		ld	c,0				;CP/M disk A:
		call	seldsk
		ld	bc,0			;CP/M boot track
		call	settrk
		ld	bc,0			;CP/M boot sector
		call	setsec
...
putsys:
		...
		ld	bc,0		;CP/M track
		call	settrk
;Write track 0, sectors 1 to cpmsectors
		ld	bc,1		;starting sector
		ld	(sector),bc
...
osloader:    
        INCBIN "../binaries/cpmloader8M.bin"
osloader_end:
...
; -------------------------------------------------------------
cbioscode_start:    
        INCBIN "../binaries/cbios-ide-8M.bin"
cbioscode_end:		
