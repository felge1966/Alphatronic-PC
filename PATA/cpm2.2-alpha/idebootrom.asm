...
brd_status_loop_1:
			...
			jp	nz,brd_status_loop_1		;loop until not busy
brd_status_loop_2:
			in	a,(pata_stat)	;check	status
			and	40h						;check DRDY bit
			jp	z,brd_status_loop_2		;loop until ready
			...
			out	(pata_cmd),a
brd_wait_for_DRQ_set:
			in	a,(pata_stat)			;read status
			and	08h						;DRQ bit
			jp	z,brd_wait_for_DRQ_set	;loop until bit set
brd_wait_for_BSY_clear:
			in	a,(pata_stat)			;read status
			and	80h
			jp	nz,brd_wait_for_BSY_clear
			in	a,(pata_stat)			;clear INTRQ
brd_read_loop:
			in	a,(pata_data)			;get data
			ld	(hl),a
			inc	hl
			in	a,(pata_stat)			;check status
			and	08h						;DRQ bit
			jp	nz,brd_read_loop		;loop until clear
...
brd_sector_loop:
			...
			inc	hl
			inc	de
			djnz	brd_sector_loop		;put 128 bytes into memory
...
hstbuf: 	ds 256	;buffer to hold raw sector read from disk (256 bytes)
tmpbuf:		ds 256	;temporary buffer for disk read operations (256 bytes)
